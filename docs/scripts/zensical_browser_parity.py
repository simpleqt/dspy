...
SEARCH_QUERIES = ("dspy.configure_cache", "MIPROv2", "saving and loading")
SEARCH_EXPECTATIONS = {
    "dspy.configure_cache": "/current/api/utils/configure_cache/",
    "MIPROv2": "/current/api/optimizers/MIPROv2/",
    "saving and loading": "/current/tutorials/saving/",
}
...
def exercise(page: Page, base_url: str) -> dict[str, object]:
    ...
    page.get_by_role("link", name="3.3.1", exact=True).click()
    page.wait_for_url("**/3.3.1/")
    version_navigation = urlparse(page.url).path == "/3.3.1/"
    stable_page(page, f"{base_url}/current/")
    ...
    return {
        ...
        "version_navigation": version_navigation,
