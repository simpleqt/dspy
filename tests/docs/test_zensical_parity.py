...
<body>navigation text<article><h1 id="guide">Guide</h1><h4 id="dspy.LM.call">call(x: str)</h4><p>Useful content</p>
<script>ignored script text</script></article>footer text</body></html>"""
    ...
    assert parser.headings == [("guide", "Guide"), ("dspy.LM.call", "call(x: str)")]
    assert parser.content_text == ["Guide", "call(x: str)", "Useful content"]
    assert "navigation text" in parser.text
    assert "ignored script text" not in parser.text


def test_parity_site_uses_the_real_mike_current_topology(tmp_path):
    pytest.importorskip("mike")
    site = tmp_path / "site"
    (site / "api").mkdir(parents=True)
    (site / "index.html").write_text("<html><head></head><body>Home</body></html>")
    (site / "api" / "index.html").write_text("<html><head></head><body>API</body></html>")
    assets = tmp_path / "assets"
    assets.mkdir()
    (assets / "releases.json").write_text(
        json.dumps(
            {"releases": [{"version": "3.3.1", "properties": {"renderer": "material", "package_source": "pypi-wheel"}}]}
        )
    )
    output = tmp_path / "deployment"

    assemble_current_site(site, output, assets, "zensical")

    assert '"current/" + window.location.search + window.location.hash' in (output / "index.html").read_text()
    assert redirect_target(output, "/api/") == "/current/api/"
    assert (output / "current" / "api" / "index.html").exists()
    assert (output / "3.3.1" / "index.html").exists()
    assert deployment_topology(output, "zensical")["passed"]


def test_search_content_combines_anchor_records_without_notebook_chrome():
    entries = [
        {"location": "guide/", "text": "Introduction"},
        {"location": "guide/#details", "text": "Detailed behavior"},
        {"location": "tutorial/", "text": "Copied useful output"},
    ]

    content = search_content(entries, {"tutorial/index.html"})

    assert content["guide/"] == {"introduction", "detailed", "behavior"}
    assert content["tutorial/"] == {"useful", "output"}
